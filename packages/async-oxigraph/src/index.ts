...
export class AsyncOxigraph {
  ...
  async load(
    ...
    return await this._runBackgroundTask({
      task: TaskType.LOAD,
      loadPayload: { triples, mimetype, baseURI, graphURI },
    });
  }
  ...
  private async _runBackgroundTask(
    ...
    return new Promise((resolve, reject) => {
      ...
      this._worker.postMessage({ ...instruction, taskID }); // Send data to our worker.
    });
  }
  ...
  private _replaceHandlebars(template: string, data: TemplateData): string {
    ...
    return template.replace(regex, (match, variableName) => {
      return data.hasOwnProperty(variableName)
        ? data[variableName].toString()
        : "";
