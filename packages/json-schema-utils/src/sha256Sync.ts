/** Sync SHA-256 hex digest for UTF-8 text (Bun fast path + browser pure-JS fallback). */
...
function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  const withOne = new Uint8Array(((message.length + 9 + 63) & ~63) >>> 0);
  withOne.set(message);
  withOne[message.length] = 0x80;
  new DataView(withOne.buffer).setUint32(
    withOne.length - 4,
    bitLen >>> 0,
    false,
  );
  ...
  for (let offset = 0; offset < withOne.length; offset += 64) {
    for (let i = 0; i < 16; i++) {
      W[i] = new DataView(withOne.buffer).getUint32(offset + i * 4, false);
    }
    for (let i = 16; i < 64; i++) {
